	...
	static public function mdlMostrarUsuarios($tabla1, $tabla2, $item, $valor){
		...
		// if($tabla2 == "empleados"){
			...
			if($item != null){
				...
				$stmt = Conexion::conectar()->prepare("SELECT p.*, e.usuario, e.password, e.foto, e.codigo, e.fecha_recuperacion, e.estado, e.primera_vez, e.bloqueado, e.id AS id_usuario, r.rol FROM $tabla1 AS p\n"
